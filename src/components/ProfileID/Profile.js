    ...
    <div className="containerProfile" key={id}>
      <div className="ProfileId">
        <div className="ProfileIdLeft">
          ...
          {verific === 1 ? (
            <p className="textUserId titleUserProf">
              {detailUser[0]?.professionals[0]?.tuition}
            </p>
          ) : null}
        ...
        <div className="ProfileIdCenter">
          <div className="barraMenuUser">
            <button
              id="userbutton"
              className={userOptionState === 1 ? "tabs set-tabs" : "tabs"}
              onClick={() => toggleTab(1)}
            >
              Informacion
            </button>
            <button
              id="userbutton"
              className={userOptionState === 3 ? "tabs set-tabs" : "tabs"}
              onClick={() => toggleTab(3)}
            >
              Publicaciones
            </button>
            {verific === 0 ? (
              <button
                id="userbutton"
                className={userOptionState === 2 ? "tabs set-tabs" : "tabs"}
                onClick={() => toggleTab(2)}
              >
              ...
              <button
                id="userbutton"
                className={userOptionState === 4 ? "tabs set-tabs" : "tabs"}
                onClick={() => toggleTab(4)}
              >
                Postulaciones
              </button>
            ...
            {verific === 0 ? (
              <Link to="/postForm">
                <button id="userbuttonRegister">Realiza un Post</button>
              </Link>
            ) : (
              <button id="userbuttonRegister">Busca una Ofertas</button>
            )}
          </div>
          <div className="containerDataUser">
            <div
              className={
                userOptionState === 1 ? "content  active-content" : "content"
              }
            >
              {detailUser.map((e) => (
                <CardUseData
            ...
            <div
              className={
                userOptionState === 2 ? "content  active-content" : "content"
              }
            >
              {detailUser.map((s) => (
                <CardRegProf key={s.id} idUs={s.id} />
              ))}
            </div>
            <div
              className={
                userOptionState === 3 ? "content  active-content" : "content"
              }
            >
              <CardPostUser />
            </div>
            <div
              className={
                userOptionState === 4 ? "content  active-content" : "content"
              }
            >
        ...
        <div className="ProfileIdRight"></div>
      </div>
    </div>
  );
}
